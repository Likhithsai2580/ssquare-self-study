...
def get_personalized_recommendations(user_id, subject):
    # Get user's exam history
    user_exams = UserExam.query.filter_by(user_id=user_id).all()
    ...
    # Add personalization based on user's weak and strong topics
    weak_topics = [exam.exam.subject for exam in user_exams if exam.score < 70]
    strong_topics = [exam.exam.subject for exam in user_exams if exam.score >= 70]
    
    personalized_recommendations = []
    for material in recommended_materials:
        if any(topic in material.content for topic in weak_topics):
            personalized_recommendations.append((material, 'weak'))
        elif any(topic in material.content for topic in strong_topics):
            personalized_recommendations.append((material, 'strong'))
        else:
            personalized_recommendations.append((material, 'neutral'))
    
    return personalized_recommendations[:5]  # Return top 5 personalized recommendations
